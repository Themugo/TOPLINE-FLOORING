...
Deno.serve(async (req) => {
  ...
  const secret = req.headers.get("x-topline-callback-secret") ?? new URL(req.url).searchParams.get("secret");
  if (secret !== callbackSecret) return new Response("Unauthorized", { status:401 });
