...
Deno.serve(async(req)=>{
 ...
 const body=await req.json().catch(()=>null) as any; const item=body?.items?.[0]||body;
 ...
 const {error}=await admin.rpc("record_inbound_communication_worker",{p_provider:"brevo_inbound",p_channel:"email",p_sender:sender,p_recipient:item?.To?.[0]?.Email||null,p_subject:item?.Subject||null,p_message:message,p_provider_message_id:item?.MessageId||null,p_payload:item});
