...
Deno.serve(async(req)=>{
 ...
 const body=await req.json().catch(()=>null) as JsonValue; const root=typeof body==='object'&&body!==null&&!Array.isArray(body)?body as JsonRecord:null; const items=root?.items; const item=Array.isArray(items)&&typeof items[0]==='object'&&items[0]!==null?items[0] as JsonRecord:root;
