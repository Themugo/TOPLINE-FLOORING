...
export default function FinanceOperations() {
  ...
  return <AdminLayout title="Finance Operations 360">
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6"><div><p className="text-sm text-navy-500">30-day billing and collections control</p><h1 className="text-2xl font-display font-bold text-navy-900">Finance Operations</h1></div><div className="flex gap-2"><button onClick={() => void reconcile()} disabled={reconciling} className="btn-secondary flex items-center gap-2 w-fit"><RefreshCw className={`w-4 h-4 ${reconciling ? 'animate-spin' : ''}`} /> {reconciling ? 'Reconciling…' : 'Reconcile finance'}</button><button onClick={() => void refresh()} disabled={refreshing} className="btn-secondary flex items-center gap-2 w-fit"><RefreshCw className={`w-4 h-4 ${refreshing ? 'animate-spin' : ''}`} /> Refresh lifecycle</button></div></div>
    ...
    <div className="mt-5 grid lg:grid-cols-3 gap-5"><section className="bg-white border rounded-xl p-5"><h2 className="font-semibold text-navy-900">Collections control</h2><p className="text-sm text-navy-500 mt-1">Invoice collections {loading ? '—' : money(controlPayments.invoice_collections)} · order collections {loading ? '—' : money(controlPayments.order_collections)}.</p></section><section className="bg-white border rounded-xl p-5"><h2 className="font-semibold text-navy-900">Refund exposure</h2><p className="text-sm text-navy-500 mt-1">Successful {loading ? '—' : money(controlRefunds.successful)} · pending {loading ? '—' : money(controlRefunds.pending)}.</p></section><section className="bg-white border rounded-xl p-5"><h2 className="font-semibold text-navy-900">Reconciliation</h2><p className="text-sm text-navy-500 mt-1">Invoice mismatches: {loading ? '—' : count(reconciliation.invoice_mismatches)} · order mismatches: {loading ? '—' : count(reconciliation.order_mismatches)}.</p></section></div>
  </AdminLayout>;
}
