...
const stages = ['pending','processing','dispatched','in_transit','delivered','failed','cancelled'];
export default function AdminDeliveries() {
  const [rows,setRows]=useState<DeliveryRow[]>([]); const [loading,setLoading]=useState(true); const {toast}=useToast();
  const load=useCallback(async()=>{setLoading(true); const {data,error}=await supabase.from('deliveries').select('*, order:orders(id,order_number,customer_name,customer_phone,total_amount,status)').order('created_at',{ascending:false}); if(error) toast({title:'Unable to load deliveries',description:error.message,variant:'destructive'}); setRows(data||[]); setLoading(false);},[toast]);
  useEffect(()=>{void load();},[load]);
  const run=async(fn:()=>Promise<any>, title:string)=>{try{await fn();toast({title});await load();}catch(e){toast({title:'Delivery action failed',description:e instanceof Error?e.message:'Please try again',variant:'destructive'});}};
