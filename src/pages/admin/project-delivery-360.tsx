...
const arr = (value: unknown) => Array.isArray(value) ? value as Array<Record<string, RpcResult>> : [];

export default function AdminProjectDelivery360() {
  const [rows, setRows] = useState<Array<Record<string, RpcResult>>>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const next = (Array.isArray(data.projects) ? data.projects : []) as Array<Record<string, RpcResult>>;
      setRows(next);
