...
if (migrations.length !== 88) failures.push(`Expected 88 active migrations, found ${migrations.length}.`);
...
for (const [script, label] of scripts) {
  try {
    ...
    execFileSync(process.platform === 'win32' ? process.env.ComSpec : command,
      process.platform === 'win32' ? ['/d', '/s', '/c', `${command} run ${script}`] : ['run', script],
...
for (const requiredText of ['88 active migrations', '148 tables', '217 functions', 'db push --dry-run --linked', 'Never run `supabase db reset --linked`']) {
